...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type="50d"):
    ...
    # 2. Get embeddings for each word & 3. Add embeddings for each word
    for word in words:
        if word: # Handle empty strings from split
            word_emb = get_glove_embeddings(word, word_index_dict, embeddings, model_type)
            embedding += word_emb
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        # Compute them on the fly using the helper function.
        ...
        # Compute similarity
        for index, category in enumerate(categories):
            # Access cached category embedding
            actual_cache_key = "cat_embed_openai_" + model_name
            
            category_embedding = st.session_state[actual_cache_key][category]
            cosine_sim[index] = cosine_similarity(input_embedding, category_embedding)
        ...
        # Compute similarity
        for index, category in enumerate(categories):
            actual_cache_key = "cat_embed_transformers_" + model_name
    ...
    # Sort the results
    # cosine_sim is a dict {index: score}
    # We want a list of tuples sorted by score desc
...
if __name__ == "__main__":
    ...
    if st.session_state.text_search:
        ...
        st.write("")
        st.write(
            "Demo developed by Jimmy Yin (www.linkedin.com/in/jimmy-yin)"
        )
